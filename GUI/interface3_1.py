...
def calculate_discount_factors():
    """Calculates individual NPVs and updates the Total NPV Old."""
    try:
        ...
        for i in range(payment_year):
            if i < len(NPV_entries):
                ...
                # Calculate NPV for the year
                npv_value = total_payment * discount_factor

                # Update the entry with the NPV value
                NPV_entries[i].insert(0, f"{npv_value:.2f}")
                total_npv_old += npv_value
                NPV_entries[i].config(state="readonly")

        # Update the Total NPV Old entry box
        Total_NPV_Old_entry.config(state="normal")
        Total_NPV_Old_entry.delete(0, tk.END)
        Total_NPV_Old_entry.insert(0, f"{total_npv_old:.2f}")
        Total_NPV_Old_entry.config(state="readonly")
...
def generate_year():
    for spinbox in year_amount_spinboxs:
        spinbox.destroy()
    year_amount_spinboxs.clear()
    ...
    for x in range(year):
        ...
        year_amount_spinboxs.append(spinbox)
...
def export_to_excel(total_npv_old):
    ...
    # Handle user_data sheet
    if "User Data" not in wb.sheetnames:
        user_data_sheet = wb.create_sheet("User Data")  
    else:
        user_data_sheet = wb["User Data"]

    # Get the total NPV Old value from the Entry widget
    Total_NPV_Old_value = float(Total_NPV_Old_entry.get())

    

    next_column = 2

    # Insert the value into the next available column
    user_data_sheet.cell(row=4, column=next_column, value=Total_NPV_Old_value)

    # Get the product name from the entry widget
    product_name = product_name_entry.get()

    # Create product sheet with appropriate name
    new_sheet_name = f"{product_name} old"
    

    product_sheet = wb.create_sheet(new_sheet_name)
    product_sheet.tab_color = "FF0000"  # Set tab color to red

    # Set cell headers for calculations
    ...
    product_sheet.cell(row=27, column=1, value="Total NPV")
    product_sheet.cell(row=28, column=1, value="Total NPV old")

    # Set reference formulas
    ...
    # Write data to sheet
    for i, (year, amount, risk, out) in enumerate(zip(year_amount, amount_value, risk_amount_value, out_amount_value)):
        product_sheet.cell(row=5 + i, column=1, value=year)  # Year
        product_sheet.cell(row=5 + i, column=2, value=amount)  # Amount
        product_sheet.cell(row=5 + i, column=4, value=year)  # Risk Year
        product_sheet.cell(row=5 + i, column=5, value=risk)  # Risk Amount
        product_sheet.cell(row=5 + i, column=7, value=year)  # Out Year
        product_sheet.cell(row=5 + i, column=8, value=out)  # Out Amount
    ...
    for i in range(2, 16):
        ...
        product_sheet.cell(
            row=22,
            column=i,
            value=f'=IF(SUM({column_letter}19:{column_letter}21)=0, "", 1/SUM({column_letter}19:{column_letter}21))',
        )

    # Write discount factor formulas
    for i in range(14):
        column_letter = chr(66 + i)
        product_sheet.cell(
            row=22, column=i + 2, value=f'=IF({column_letter}14="", "", (1/(1+${column_letter}$13)^{column_letter}14))'
        )

    # Write totals and additional data
    ...
    # Write discount rates and payment years
    for i, rate in enumerate(discount_rates):
        product_sheet.cell(row=13, column=2 + i, value=rate)
    for i in range(payment_years):
        product_sheet.cell(row=14, column=2 + i, value=i + 1)

    # Write payment patterns
    for i, pattern in enumerate(payment_patterns):
        product_sheet.cell(row=17, column=2 + i, value=pattern)

    # Write NPV values
    npv_values = [entry.get() for entry in NPV_entries]
    for i, value in enumerate(npv_values):
        product_sheet.cell(row=27, column=2 + i, value=value)

    # Write total NPV old only once
    product_sheet.cell(row=28, column=2, value=total_npv_old)
    ...
    try:
        ...
        print("Error: Could not save the file. Please ensure the Excel file is not open in another program.")
...
def next_interface():
    try:
        # Initialize Total_NPV_old to 0 or another appropriate starting value
        # Proceed to the next interface
        total_npv_old =Total_NPV_Old_entry.get()
        ...
        if "User Data" in wb.sheetnames:
            ...
            user_data_sheet.cell(row=4, column=17, value="=SUM(A4:P4)")
        ...
        export_to_excel(total_npv_old)

        # Close current interface
        interface.destroy()

        # Launch next interface
        subprocess.Popen(["python", r"C:\GI_Automation\GUI\interface3.py"])
...
def back():
    ...
    interface.destroy()
    subprocess.Popen(["python", r"C:\GI_Automation\GUI\interface2.py"])


# Setup main window
interface = tk.Tk()
interface.title("Interface3.1")
interface.geometry("2500x1000")

frame = tk.Frame(interface)
frame.pack()

user_info_frame = tk.LabelFrame(frame, text="Subsq old rate", padx=10, pady=10)
...
NPV_label = tk.Label(user_info_frame, text="Net Present Value Old:")
NPV_label.grid(row=27, column=0, sticky="w")

# Total Net Present Value Old
Total_NPV_Old_label = tk.Label(user_info_frame, text="Total Net Present Value Old:")
Total_NPV_Old_label.grid(row=28, column=0, sticky="w")

# Entry for Total Net Present Value Old
Total_NPV_Old_entry = tk.Entry(user_info_frame, width=20)
Total_NPV_Old_entry.grid(row=28, column=1)
Total_NPV_Old_entry.config(state="readonly")  # Set to readonly initially
...
export_button = tk.Button(
    ...
    command=lambda: export_to_excel(Total_NPV_Old_entry.get()),
)
